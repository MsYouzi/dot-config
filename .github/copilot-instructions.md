...
Keep the status-line layout and cache aligned. Preserve the documented provider metrics and live-subagent differences. Fork colors come from `scripts/theme/catppuccin-mocha.*` through `scripts/catppuccin-theme.sh`; do not restore retired theme paths or embed colors in active consumers.
